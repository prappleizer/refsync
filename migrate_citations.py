...
By default, looks for the database in ~/.refsync/library.db
Set REFSYNC_DATA_DIR to use a different location.
"""
...
def get_db_path():
    """Find the database in the RefSync data directory."""
    if env_dir := os.environ.get("REFSYNC_DATA_DIR"):
        data_dir = Path(env_dir)
    else:
        data_dir = Path.home() / ".refsync"
...
def generate_cite_key(arxiv_id: str, authors: list, published: datetime, existing_keys: set) -> str:
...
def generate_arxiv_bibtex(
    arxiv_id: str, title: str, authors: list, categories: list, published: datetime, cite_key: str
) -> str:
...
def migrate_papers(conn: sqlite3.Connection):
    ...
    updated = 0
    for row in papers:
        arxiv_id, title, authors_json, categories_json, published_str, cite_key, bibtex = row
